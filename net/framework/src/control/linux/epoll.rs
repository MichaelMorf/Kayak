...
impl PollHandle {
    ...
    pub fn schedule_read_rawfd(&self, fd: RawFd, token: Token) {
        let mut event = EpollEvent::new(EpollFlags::EPOLLIN |
                                        EpollFlags::EPOLLET |
    ...
    pub fn schedule_write_rawfd(&self, fd: RawFd, token: Token) {
        let mut event = EpollEvent::new(EpollFlags::EPOLLOUT |
                                        EpollFlags::EPOLLET |
                                        EpollFlags::EPOLLONESHOT,
                                        token);
        epoll_ctl(self.epoll_fd, EpollOp::EpollCtlMod, fd, &mut event).unwrap();
    ...
    pub fn new_io_fd(&self, fd: RawFd, token: Token) {
        let mut event = EpollEvent::new(EpollFlags::EPOLLET |
                                        EpollFlags::EPOLLONESHOT,
                                        token);
...
impl PollScheduler {
    ...
    fn epoll_kind_to_available(&self, kind: &EpollFlags) -> Available {
        ...
        if kind.contains(EpollFlags::EPOLLHUP) ||
                         kind.contains(EpollFlags::EPOLLERR) {
            available |= HUP
        };
        available
    }
